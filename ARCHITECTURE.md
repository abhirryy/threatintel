...
It refreshes **every 5 minutes**, is hosted **for free** on your own domain, and is built so you can bolt on more sections over time.
...
## 5. The 5-minute update

A GitHub Actions workflow with `schedule: cron('*/5 * * * *')` runs the aggregator, and commits the refreshed `data/feed.json`. The commit triggers a Pages redeploy automatically. You can also trigger it by hand from the Actions tab (`workflow_dispatch`).

Note: GitHub's cron can lag a few minutes under load — fine for a 5-minute cadence.
...
├── .github/workflows/
│   └── update.yml          # runs every 5 minutes
